export default function comparing(curProd = [], relProd = []) {
  ...
  const curProdChars = curProd.map((char) => (!char.value ? `${char.feature}` : `${char.value} ${char.feature}`));
  const relProdChars = relProd.map((char) => (!char.value ? `${char.feature}` : `${char.value} ${char.feature}`));
