...
import StyleSelector from '../../../src/components/Products/StyleSelector';
...
test('should display the Selected Style in the heading', () => {
  render(<StyleSelector styles={styles} curStyle={curStyle} />);
  const heading = screen.getByRole('heading', { name: /selected style > black/i });
  expect(heading.innerHTML).toBe('Selected Style &gt; Black ');
});

test('should display the Selected Style in the heading', () => {
  render(<StyleSelector styles={styles} setStyles="true" curStyle={curStyle} />);
  const thumbnail = screen.getByRole('img', { name: /white/i });
  expect(thumbnail.alt).toBe('White');
});
